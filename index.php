    ...
    if ($itemCount > 0) {
        ...
        if (db_table_exists($pdo, 'authors') && db_table_exists($pdo, 'item_authors')) {
            ...
            if ($authorCandidates !== []) {
                ...
                $authorSection = [
                    'name' => (string)$picked['name'],
                    'url' => public_url('search.php') . '?' . http_build_query(['q' => (string)$picked['name']]),
                    'items' => $authorItems,
                ];
            }
        }
...
<?php else: ?>
  ...
  <section class="rail-section home-genre-section">
    ...
    <?php foreach ($genreRows as $genre): ?>
      <h3><a href="<?= e(public_url('search.php') . '?' . http_build_query(['q' => (string)$genre['name']])) ?>"><?= e((string)$genre['name']) ?></a></h3>
      <div class="rail-row rail-row--200 rail-row--wide-thumb rail-row--bottom-scroll rail-row--bottom-horizontal rail-row--home-taxonomy">
        <?php foreach ($genre['items'] as $item) { render_item_card($item, 200, ['name' => (string)$genre['name'], 'url' => public_url('search.php') . '?' . http_build_query(['q' => (string)$genre['name']])], true); } ?>
      </div>
    <?php endforeach; ?>
  </section>
  <?php if (!empty($seriesSection['items'])): ?>
  <section class="rail-section">
    <h2>シリーズ<?= $seriesSection['name'] !== '' ? '：<a href="' . e(public_url('search.php') . '?' . http_build_query(['q' => (string)$seriesSection['name']])) . '">' . e($seriesSection['name']) . '</a>' : '' ?></h2>
    <div class="rail-row rail-row--200 rail-row--wide-thumb rail-row--bottom-scroll rail-row--bottom-horizontal rail-row--home-taxonomy">
      <?php foreach ($seriesSection['items'] as $item) { render_item_card($item, 200, ['name' => (string)$seriesSection['name'], 'url' => public_url('search.php') . '?' . http_build_query(['q' => (string)$seriesSection['name']])], true); } ?>
    </div>
  </section>
  <?php endif; ?>
  ...
  <section class="rail-section">
    <h2>メーカー<?= $makerSection['name'] !== '' ? '：<a href="' . e(public_url('search.php') . '?' . http_build_query(['q' => (string)$makerSection['name']])) . '">' . e($makerSection['name']) . '</a>' : '' ?></h2>
    <div class="rail-row rail-row--200 rail-row--wide-thumb rail-row--bottom-scroll rail-row--bottom-horizontal rail-row--home-taxonomy">
      <?php foreach ($makerSection['items'] as $item) { render_item_card($item, 200, ['name' => (string)$makerSection['name'], 'url' => public_url('search.php') . '?' . http_build_query(['q' => (string)$makerSection['name']])], true); } ?>
    </div>
  </section>
  <?php endif; ?>
